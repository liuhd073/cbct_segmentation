...
model = vnet(input_channel, 64, numofclasses)
        ...
        for j in range(len(subject_list)):
            ...
            for i_batch in range(int(new_vertex.shape[0]/batch_size)):
                ...
                for count in range(batch_size):
                    ...
                    class_gt = np.argmax(seg_gt_one, axis=0)
...
def Train():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i_iter in range(iterations):
            ...
            loss, loss_seg = model.train_on_batch([image1, batch_label], [])

            if (i_iter+1) % 200 == 0:
                print('Epoch:{0:2d}, iter = {1:5d}, loss = {2:.4f}, loss_seg = {3:.4f}'.format(epoch+1, i_iter, loss, loss_seg))
        
        # Validation
        loss_sum = 0.
        for vi_iter in range(val_iterations):
            [image1, batch_label], _ = next(val_gen)
            val_loss, val_loss_seg = model.test_on_batch([image1, batch_label], [])
