...
def test(batch_size=1, window_str=8, stride_str=8):
    ...
    model = vnet(input_channel, 64, numofclasses, batch_size=1, window=window_str, stride=stride_str, lambda_seg=0.5, lambda_sem=0.5)
    ...
    for j in range(len(test_subject_list)): ## should be changed to 2 ##
        ...
        for idx in range(total_num_of_patches):
            ...
            fake_class_gt = np.zeros([batchsize, 64, numofclasses])
            ## output batch ##
            predicted_one, _, _, _ = model.predict([image_1, fake_gt, fake_class_gt])
...
test(batch_size=batchsize, window_str=window_str, stride_str=stride_str)
